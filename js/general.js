...
function modalControl() {
  ...
  $('.slide__modal-product').on('click', function () {
    const productId = $(this).data('slide-modal-product');

    const $slides = $('.slide__products').find('[data-product-id]');
    let targetIndex = -1;
    ...
    if (targetIndex !== -1) {
      $('.slide__modal').fadeOut(500);
      $('.slide__products').slick('slickGoTo', targetIndex);
    } else {
      $('.slide__modal').fadeOut(500);
    }
  });
...
function initSlickSlider() {
  ...
  $('.slide--product-buttons button').on('click', function () {
    $('.slide__products-content').removeClass('error');
    const button = $(this);
    ...
    currentSlideNum = slideContainer.getAttribute('data-current-slide');

    if (allowedIds.includes(productId)) {
      needbleSlot.find(`.slide__slot-siluet-product--${productId}`).addClass('remove-filter');
      ...
      if (Number(currentSlideNum) === 5 && needbleSlot.hasClass('slide__slot--day') && (productId === 6 || productId === 14)) {
        if (!needbleSlot.find('.alt-products').length) {
          $('<p>', {
            class: 'alt-products',
            text: 'альтернативні засоби'
          }).insertAfter(newProduct);
        }
      }
    ...
    needbleSlot.attr('data-correct-products', JSON.stringify(updateAllowedIds));
      ...
      $('.slide__products-content').addClass('error');
    ...
    if (!$('.slide__slot').hasClass('half-active')) {
      $.when(
        ...
        $('.slide__products .slick-next').fadeOut(300)
      ).done(() => {
        $('button.open-modal').addClass('solved');
        slideSolvedAction(Number(currentSlideNum));
      });
    }
  });
}
